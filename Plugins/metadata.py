...
async def handle_metadata(bot: Client, message: Message):
    ...
    bool_metadata = await Kim_Dokja14.get_metadata(message.from_user.id)
    user_metadata = await Kim_Dokja14.get_metadata_code(message.from_user.id)
    await ms.delete()
    if bool_metadata:
...
async def query_metadata(bot: Client, query: CallbackQuery):
    ...
    if data.startswith('metadata_'):
        ...
        user_metadata = await Kim_Dokja14.get_metadata_code(query.from_user.id)

        if bool(eval(_bool)):
            await Kim_Dokja14.set_metadata(query.from_user.id, bool_meta=False)
            await query.message.edit(f"**Your Current Metadata :-**\n\n➜ `{user_metadata}` ", reply_markup=InlineKeyboardMarkup(OFF))

        else:
            await Kim_Dokja14.set_metadata(query.from_user.id, bool_meta=True)
        ...
        try:
            ...
            await Kim_Dokja14.set_metadata_code(query.from_user.id, metadata_code=metadata.text)
